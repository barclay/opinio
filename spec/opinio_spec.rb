...
describe Opinio do
  ...
  it "should accept custom destroy conditions for the opinio model" do
    ...
    controller.can_destroy_opinio?(comment).should be
    controller.can_destroy_opinio?(comment_owned_by_someone_else).should eq(false)
  end
  ...
  it "should accept custom destroy conditions with a block" do
    ...
    controller = ActionController::Base.new
    expect(controller.can_destroy_opinio?(double("comment"))).to eq(false)

    ActionController::Base.comment_destroy_conditions do
      true 
    end
    controller = ActionController::Base.new
    expect(controller.can_destroy_opinio?(double("comment"))).to eq(true)
